...
// Alternative simpler conversion function
function safeConvertToPlainObject(doc) {
    if (!doc) return null;

    const converted = JSON.parse(JSON.stringify(doc));

    // Ensure _id is properly converted
    if (converted && converted._id && converted._id.$oid) {
...
async function getCourseData(courseId, userEmail) {
   // console.log(courseId);
    try {
        ...
        const progressCollection = await dbConnect(
            collectionNamesObj.progressBarCollection
        );
        ...
        const progressData = await progressCollection.findOne({
            userId: userEmail,
            courseId: courseId
        });

        // Convert all data to plain objects using the simpler function
        const plainCourse = safeConvertToPlainObject(course);
        const plainProgress = safeConvertToPlainObject(progressData);

        // Ensure _id is properly set
        if (plainCourse && plainCourse._id) {
            plainCourse._id = plainCourse._id.toString ? plainCourse._id.toString() : String(plainCourse._id);
        }

        return {
            ...plainCourse,
            userProgress: plainProgress || null
        };
    } catch (error) {
...
export default async function CourseDetailPage({ params }) {
    ...
    // Safely handle course ID
    const courseId = course._id ? course._id.toString() : id;

    // Calculate overall progress
    const overallProgress = course.userProgress?.progress || 0;
    // const totalModules = course.milestones?.flatMap(m => m.modules).length || 0;
    // const completedModules = course.userProgress?.watchedVideos?.length || 0;

    // Prepare plain JavaScript objects for client components
    const plainMilestones = (course.milestones || []).map(milestone => ({
        ...milestone,
        modules: (milestone.modules || []).map(module => ({
            ...module,
            // Ensure all module properties are plain values
            _id: module._id ? (module._id.toString ? module._id.toString() : String(module._id)) : `module-${Math.random()}`,
            video: module.video || '',
            title: module.title || '',
    ...
    const plainUserProgress = course.userProgress ? {
        ...course.userProgress,
        _id: course.userProgress._id ? (course.userProgress._id.toString ? course.userProgress._id.toString() : String(course.userProgress._id)) : null,
        userId: course.userProgress.userId,
        courseId: course.userProgress.courseId,
        progress: course.userProgress.progress || 0,
        watchedVideos: course.userProgress.watchedVideos || [],
        lastWatchedModule: course.userProgress.lastWatchedModule,
        createdAt: course.userProgress.createdAt,
        updatedAt: course.userProgress.updatedAt
    } : null;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="    mb-6">
                            ...
                            <VideoSection
                                pasCourses={plainMilestones}
                                modules={plainMilestones.flatMap(m => m.modules)}
                            ...
                            <div className="bg-white rounded-lg shadow-md p-6">
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-gray-600 text-justify mt-2">{course.instructor.bio}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 bg-white rounded-lg shadow-lg p-6 space-y-6">
                            ...
                            <div className="pt-4 border-t border-gray-200">
                                <h4 className="font-semibold mb-3">Milestone Progress</h4>
                                <div className="space-y-3">
                                    {plainMilestones?.map((milestone, index) => {
                                        const milestoneModules = milestone.modules || [];
                                        const completedMilestoneModules = milestoneModules.filter((mod, i) => {
                                            const moduleIndex = plainMilestones
                                                .flatMap(m => m.modules)
                                                .findIndex(m =>
                                                    m.title === mod.title && m.video === mod.video
                                                );
                                            return plainUserProgress?.watchedVideos?.includes(moduleIndex);
                                        }).length;

                                        const milestoneProgress = milestoneModules.length > 0
                                            ? Math.round((completedMilestoneModules / milestoneModules.length) * 100)
                                            : 0;
